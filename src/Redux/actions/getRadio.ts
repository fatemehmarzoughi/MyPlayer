...
export const getAllRadio = () => {
  return async (dispatch) => {
    const recommendedRES = await GetNoToken("/items/status/0/category/4/subCategory/0");
    const trendingNowRES = await GetNoToken("/items/status/1/category/4/subCategory/0");
    const mostWatchedRES = await GetNoToken("/items/status/2/category/4/subCategory/0");
    const newReleasesRES = await GetNoToken("/items/status/3/category/4/subCategory/0");
