...
export const getAllMostWatched = () => {
    return async (dispatch) => {
        const res = await GET_noToken('/items/status/2/category/0/subCategory/0');
        if(res.status === 200) 
        {
            const result = await res.json();
...
export const getAllTrendingNow = () => {
    return async (dispatch) => {
        const res = await GET_noToken('/items/status/1/category/0/subCategory/0');
        if(res.status === 200) 
        {
            const result = await res.json();
...
export const getAllNewReleases = () => {
    return async (dispatch) => {
        const res = await GET_noToken('/items/status/3/category/0/subCategory/0');
        if(res.status === 200) 
        {
            const result = await res.json();
